...
This doctoral thesis focused first on the usage of consumer devices -- Android smartphones and Wear OS smartwatches -- for HAR systems, to finally explore the possibility of getting rid of any wearable device, relying on infrastructure devices and Wi-Fi technology for sensing purposes.
...
Then, after performing actual experimentation, public datasets containing inertial data from smartphones and smartwatches were reviewed, putting the focus on the placement of the devices and the heterogeneity and diversity (e.g., age and gender) of the subjects that participated in the data collection. From the five identified datasets, only two of them properly reported physiological information about their participants [@vaizman2017;@arrotta2023domino], while others failed in reporting information such as age information or gender distribution [@stisen2015smart;@sztyler2016;@weiss2019smartphone]. Therefore, a data collection process was executed to build a dataset, carefully selecting the participants. As a result, the collected dataset is the dataset with data from the most diverse subjects in terms of age and most equally distributed in terms of gender, while having a moderate number of participants. 
...
Following these analyses, the lessons learned were employed to put HAR into practice by implementing a real-life mHealth system to compute the results of a mobility test automatically. The system was designed to work on consumer Android smartphones and Wear OS smartwatches, and its evaluation on a cohort of $30$ participants showed results similar to and better than other state-of-the-art proposals. This proved the feasibility of using smartphones and smartwatches as sensing devices for HAR, real-life mHealth systems.

Finally, the dissertation migrates from inertial-based to Wi-Fi CSI-based HAR, a recent trend that employs the channel status from a Wi-Fi infrastructure and promises recognition of human activities -- among other applications -- without other sensing devices. In this case, we used an ESP32 microcontroller to obtain the Wi-Fi CSI data and observed through several experiments that the collected data changes over time. Although very good results can be obtained when limiting the experiments to a determined time frame, this instability over time makes it hardly portable to real-life scenarios.
...
### RO1: Provide tools for reliable data collection in Android smartphones and Wear OS smartwatches
@sec-collection_tools described the *Background Sensors*, *WearOS Sensors* and *NativeScript WearOS Sensors* libraries, which grant access to some specific sensors in Android smartphones and smartwatches. 

The libraries were implemented following the guidelines presented by @gonzalez2022using, which proved to achieve a reliable task execution (i.e., data collection) with less than $1\%$ of missing data. In addition, the libraries were integrated into the AwarNS Framework, a modular framework to ease the development of context-aware applications [@gonzalez2023awarns].

### RO2: Collect HAR dataset of smartphone and smartwatch data from heterogeneous subjects
@sec-dataset described a dataset containing accelerometer and gyroscope samples from a smartphone and a smartwatch associated with five human activities. The dataset was collected focusing on the heterogeneity and diversity of the participants, resulting in the dataset of these characteristics with the wider age range of its subjects and with a balance of $56\%$/$44\%$ male/female participants.
...
[Multidimensional analysis of ML and DL on HAR](03_analysis-har.qmd) presented an analysis exploring how the classification performance on four ML and DL models and three data sources were affected by the amount of data used for training them.

Results unveiled different patterns in how the performance of classification models evolves depending on the amount of training data and the data source employed. Regarding data sources, the smartwatch data provided the best results when the amount of available data was low, while the fusion of smartphone and smartwatch data generally obtained the best results as the amount of data increased. When focusing on the models, the CNN showed the best results in almost any circumstance, making it the clear choice -- or at least, the starting point -- for HAR systems.

### RO4: Development and evaluation of an mHealth system using HAR with smartphones and smartwatches
[HAR in mHealth: TUG test using smartphones and smartwatches](04_tug.qmd) presented an mHealth system to automate the execution of the TUG test was implemented and evaluated. The system, composed of two applications, employed the inertial data from a smartphone or a smartwatch to automatically compute the test's results.

The system was evaluated on $30$ test subjects, showing an average error of $44$ ms and $51$ ms when computing the total duration of the test with the smartwatch and smartphone inertial data, respectively. In general, results indicated that the system performed better with the smartwatch data than with the smartphone data. These results showed that HAR with consumer smartphones and smartwatches can successfully be used for real-life mHealth -- which require a high degree of reliability -- applications.
...
A first experiment showed a downgrade in the classification accuracy of a CNN model when evaluating it using data from a different time span than the one used for training it. A further experiment indicated a clear similarity between the CSI data of adjacent days while being different from other days. These results suggested the instability of the CSI data, thus hampering its usage for real-life applications.
...
- "Implementing and evaluating the Timed Up and Go test automation using smartphones and smartwatches". In: **IEEE Journal of Biomedical and Health Informatics** **[Submitted. Under first review round]** [@matey2024implementing].
