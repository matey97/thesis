...
## ML and DL
In the context of HAR and thorough this thesis, **labelled** data is employed to train ML or DL models and to determine --**classify**-- the activity a user is performing given some input data. Therefore, this dissertation faces a **supervised learning classification** problem.
...
The results of the output layer can be also expressed as follows:
\begin{equation}
...
@fig-cnn shows the architecture of a CNN network. Since the convolutional layers act as feature extractors, these networks contain after the convolutional layers and a flattening layer to reduce the convolved output to one dimension, a classifier or a regressor (e.g., MLP).

#### Long-Short Term Memory (LSTM)
When the data has a sequential nature (e.g., time-series), RNN can be useful since they can remember information from the previous status and use it for the current status. This is possible because the information flows between neurons in the same and other layers. However, the learning, driven by the backpropagation algorithm, suffers from several issues such as the vanishing and exploding gradient.
...
The architecture of the CNN-LSTM network is depicted in @fig-cnnlstm. The input of the network consists of convolutional layers for spatial modelling. Then, the outputs of those convolutional layers go through several LSTM layers for modelling temporal dependencies. Finally, the outputs of the LSTM layers are fed into a MLP to perform classification.
...
$$
    ...
    \end{cases},
$$
where $\mu$ represents the mean of the population and $x$ the specific value.

##### Two-sample T-test
It is used to determine if the difference between two groups is statistically significant. It can be used to know if two groups are different and in consequence, determine which group is better. Its hypothesis can be defined as:
$$
    \begin{cases}
        H_0: \mu_A = \mu_B \\
        H_1: \mu_A \neq \mu_B
...
[^1]: Parametric tests operate with the mean of the distributions.
...
$$
    ...
    \end{cases},
$$
where $\eta$ represents the median of the population.

[^2]: Non-parametric tests usually operate with the median of the distributions.

#### Mann-Whitney U-test (MWU)
The MWU is the **non-parametric** counterpart of the two-sample T-test [@mann1947test]. It should be applied to compare two non-normally distributed groups. Its hypothesis can be defined as:
$$
    \begin{cases}
        H_0: \eta_A = \eta_B \\
        H_1: \eta_A \neq \eta_B
